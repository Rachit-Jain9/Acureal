...
} = require('../services/exports/xlsx/templates/blankTemplateContext');
...
// ─── Reference template library (PR-TPL) ─────────────────────────────────────
//
// Blank, downloadable Excel templates for 19 Indian real-estate asset classes.
// Each template is generated by feeding `buildDealWorkbookV2` a synthetic
// blank context with realistic Bengaluru-priority defaults — so the operator
// downloads a fully-structured, formula-driven, investor-grade workbook with
// all the same charts / conditional formatting / named ranges / India-context
// (RERA / GST / stamp duty / JDA / LTCG) logic as a real deal export.
//
// Routes:
//   GET /exports/templates/catalog
//     → JSON catalog of all asset classes + their supported deal structures
//        + exit strategies + India market context. Frontend uses this to
//        render the templates page.
//
//   GET /exports/templates/:assetClass/:dealStructure/xlsx
//     → Download a blank template workbook (200 OK, .xlsx attachment).
//        e.g. /exports/templates/residential_apartments/jda_revenue_share/xlsx

// GET /exports/templates/catalog — list all available templates
router.get('/templates/catalog', authenticate, async (req, res, next) => {
  try {
...
// GET /exports/templates/:assetClass/:dealStructure/xlsx — download a template
router.get(
  '/templates/:assetClass/:dealStructure/xlsx',
  authenticate,
  async (req, res, next) => {
    try {
      const { assetClass, dealStructure } = req.params;
      const exitStrategy = req.query.exit || undefined;

      if (!ASSET_CLASS_REGISTRY[assetClass]) {
        return res.status(404).json({
          success: false,
          message: `Asset class "${assetClass}" not in template catalog.`,
          validAssetClasses: Object.keys(ASSET_CLASS_REGISTRY),
        });
      }
      const meta = ASSET_CLASS_REGISTRY[assetClass];
      if (!meta.supportedDealStructures.includes(dealStructure)) {
        return res.status(400).json({
          success: false,
          message: `Deal structure "${dealStructure}" not supported for ${meta.label}.`,
          supportedDealStructures: meta.supportedDealStructures,
        });
      }

      const ctx = buildBlankTemplateContext(assetClass, dealStructure, exitStrategy);
      const xlsxBuffer = await buildDealWorkbookV2(ctx, {
        brandName: 'REDIP',
        userName: req.user?.name || 'REDIP',
        generatedAt: new Date().toISOString(),
        strictValidation: false, // templates use synthetic defaults — skip strict QA
      });

      const safeAsset = assetClass.replace(/[^a-z0-9]/gi, '-');
      const safeStruct = dealStructure.replace(/[^a-z0-9]/gi, '-');
      const filename = `redip-template-${safeAsset}-${safeStruct}.xlsx`;
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
