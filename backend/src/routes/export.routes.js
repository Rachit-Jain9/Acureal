...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const riskCounts = exportContext.risks.summary;
      const nearbyComps = exportContext.market.exportComps;

      // Risk detail rows for the Risks sheet
      const riskDetailRows = exportContext.risks.items;

      // DD detail rows
      const ddDetailRows = exportContext.dd.items;

      // AI insights — fire-and-survive. Never blocks the export.
      const insights = exportContext.ai;
      const benchmarks = exportContext.market.benchmarks;

      const wb = XLSX.utils.book_new();
      wb.Props = {
        Title: `REDIP — ${d.name || 'Deal'} — Investor Package`,
        Author: req.user.name || 'REDIP',
        Company: 'REDIP — Real Estate Development Intelligence Platform',
        CreatedDate: new Date(),
      };

      const fmtCr = (v) => (v != null && v !== '' ? Number(v) : null);
      const fmtPct = (v) => (v != null && v !== '' ? Number(v) : null);
      const durationYears = exportContext.durationYears;

      // Helper: build a sheet from a 2D AoA with column widths, freeze panes,
      // optional merged cells, and a number-format mask per column.
      const buildSheet = (rows, opts = {}) => {
        const ws = XLSX.utils.aoa_to_sheet(rows);
        if (opts.cols) ws['!cols'] = opts.cols;
        if (opts.merges) ws['!merges'] = opts.merges;
        if (opts.freeze) {
          ws['!freeze'] = opts.freeze;
          ws['!views'] = [{ state: 'frozen', ySplit: opts.freeze.ySplit || 0, xSplit: opts.freeze.xSplit || 0 }];
        }
        if (opts.formats) {
          for (const { col, rowStart = 1, rowEnd = rows.length - 1, z } of opts.formats) {
      ...
      // ─────────────────────────────────────────────────────
      const coverRows = [
        ['REDIP — Deal Investor Package'],
        [''],
        ['Deal Name', d.name || '—'],
        ['Asset Class', (d.asset_class || '').replace(/_/g, ' ')],
        ['Deal Type', (d.deal_type || '').replace(/_/g, ' ')],
        ...
        ['Land Area (sqft)', fmtCr(d.land_area_sqft)],
        ['RERA', d.rera_number || 'Not registered'],
        [''],
        ['Headline KPIs'],
        ['IRR (%)', fmtPct(d.irr_pct)],
        ['NPV (₹ Cr)', fmtCr(d.npv_cr)],
        ['Equity Multiple', fmtCr(d.equity_multiple)],
        ['Gross Margin (%)', fmtPct(d.gross_margin_pct)],
        ['Residual Land Value (₹ Cr)', fmtCr(d.residual_land_value_cr)],
        [''],
        ['Project Duration (years)', durationYears],
        ['Effective Date', d.model_params?.inputs?.effectiveDate || '—'],
        [''],
        ['Generated At', new Date().toISOString()],
        ['Generated By', req.user.name],
        ['Platform', 'REDIP — Real Estate Development Intelligence'],
      ];
      const coverWs = buildSheet(coverRows, {
        cols: [{ wch: 32 }, { wch: 50 }],
        merges: [
          { s: { r: 0, c: 0 }, e: { r: 0, c: 1 } },
        ...
        formats: [
          { col: 1, rowStart: 14, rowEnd: 18, z: '#,##0.00' }, // KPIs
          { col: 1, rowStart: 20, rowEnd: 20, z: '0.00' },      // duration
          { col: 1, rowStart: 10, rowEnd: 10, z: '#,##0' },     // land area
        ],
      });
      XLSX.utils.book_append_sheet(wb, coverWs, 'Cover');

      // ─────────────────────────────────────────────────────
      // SHEET 2 — EXECUTIVE SUMMARY (with AI IC opinion)
      // ─────────────────────────────────────────────────────
      const ddTotal = Number(ddCounts.total_required || 0);
      const ddDone = Number(ddCounts.completed_required || 0);
      const ddPct = ddTotal > 0 ? Math.round((ddDone / ddTotal) * 100) : null;

      const execRows = [
        ['Executive Summary'],
        [''],
        ['Category', 'Metric', 'Value'],
        ['Returns', 'IRR (%)', fmtPct(d.irr_pct)],
        ['Returns', 'NPV (₹ Cr)', fmtCr(d.npv_cr)],
        ['Returns', 'Equity Multiple', fmtCr(d.equity_multiple)],
        ['Returns', 'Residual Land Value (₹ Cr)', fmtCr(d.residual_land_value_cr)],
        ['P&L', 'Total Revenue (₹ Cr)', fmtCr(d.total_revenue_cr)],
        ['P&L', 'Total Cost (₹ Cr)', fmtCr(d.total_cost_cr)],
        ['P&L', 'Gross Profit (₹ Cr)', fmtCr(d.gross_profit_cr)],
        ['P&L', 'Gross Margin (%)', fmtPct(d.gross_margin_pct)],
        ['Diligence', 'DD Completion (%)', ddPct],
        ['Diligence', 'Open Deal-Breakers (DD)', Number(ddCounts.open_deal_breakers || 0)],
        ['Risk', 'Critical Risks', Number(riskCounts.critical || 0)],
        ['Market', 'Benchmark Median Rate (INR/sqft)', fmtCr(benchmarks.median_rate_per_sqft)],
        ['Market', 'Benchmark Sample Size', Number(benchmarks.count || 0)],
        ['Decision', 'Recommendation', exportContext.risks.recommendation.label],
        ['Risk', 'High Risks', Number(riskCounts.high || 0)],
        ['Risk', 'Medium Risks', Number(riskCounts.medium || 0)],
        ['Risk', 'Low Risks', Number(riskCounts.low || 0)],
        [''],
      ];
      ...
      if (insights.available && insights.ic_opinion) {
        ...
        execRows.push(['']);
        if (insights.top_risks && insights.top_risks.length) {
          execRows.push(['Top Risks (AI)']);
          for (const r of insights.top_risks) {
            execRows.push([`• ${r.title}`, r.detail]);
          }
          execRows.push(['']);
        }
        if (insights.next_steps && insights.next_steps.length) {
          execRows.push(['Recommended Next Steps (AI)']);
          for (const s of insights.next_steps) {
            execRows.push([`• ${s}`]);
          }
          execRows.push(['']);
        }
      ...
      const execWs = buildSheet(execRows, {
        cols: [{ wch: 16 }, { wch: 44 }, { wch: 22 }],
        freeze: { ySplit: 3 },
        formats: [
          { col: 2, rowStart: 3, rowEnd: 6, z: '#,##0.00' },    // returns
          { col: 2, rowStart: 7, rowEnd: 10, z: '#,##0.00' },   // P&L
          { col: 2, rowStart: 11, rowEnd: 11, z: '0"%"' },      // DD%
        ],
      });
      ...
      // SHEET 3 — ASSUMPTIONS
      // ─────────────────────────────────────────────────────
      const inputs = (d.model_params && d.model_params.inputs) || {};
      const assumptionRows = [
        ['Model Assumptions'],
        [''],
        ['Category', 'Parameter', 'Value', 'Unit / Note'],
        ['Site', 'Plot Area', fmtCr(d.plot_area_sqft), 'sqft'],
        ['Site', 'FSI / FAR', fmtCr(d.fsi), 'ratio'],
        ['Site', 'Loading Factor', fmtCr(d.loading_factor), 'fraction (0.15 = 15%)'],
        ['Pricing', 'Selling Rate', fmtCr(d.selling_rate_per_sqft), '₹ / sqft'],
        ['Pricing', 'Construction Cost', fmtCr(d.construction_cost_per_sqft), '₹ / sqft saleable'],
        ['Pricing', 'Circle Rate', fmtCr(d.circle_rate_per_sqft), '₹ / sqft'],
        ['Costs', 'Land Cost', fmtCr(d.land_cost_cr), '₹ Cr'],
        ['Costs', 'Construction Cost', fmtCr(d.construction_cost_cr), '₹ Cr'],
        ['Costs', 'Approval Cost', fmtCr(d.approval_cost_cr), '₹ Cr'],
        ['Costs', 'Marketing Cost', fmtCr(d.marketing_cost_cr), '₹ Cr'],
        ['Costs', 'Finance Cost', fmtCr(d.finance_cost_cr), '₹ Cr'],
        ['Costs', 'GST Cost', fmtCr(d.gst_cost_cr), '₹ Cr'],
        ['Costs', 'Stamp Duty', fmtCr(d.stamp_duty_cr), '₹ Cr'],
        ['Timing', 'Effective Date', inputs.effectiveDate || '—', 'start of model'],
        ['Timing', 'Project Duration', durationYears, 'years'],
        ['Returns', 'Discount Rate', fmtCr(d.discount_rate_pct), '%'],
        ['Returns', 'Developer Margin', fmtCr(d.developer_margin_pct), '%'],
      ];
      for (const assumption of exportContext.assumptions) {
        if (assumptionRows.some((row) => row[1] === assumption.label)) continue;
        assumptionRows.push(['Model Input', assumption.label, assumption.value, 'stored input']);
      }
      const assumptionsWs = buildSheet(assumptionRows, {
        cols: [{ wch: 14 }, { wch: 26 }, { wch: 16 }, { wch: 28 }],
        freeze: { ySplit: 3 },
        autofilter: 'A3:D3',
        formats: [
          { col: 2, rowStart: 3, rowEnd: assumptionRows.length - 1, z: '#,##0.00' },
        ],
      });
      XLSX.utils.book_append_sheet(wb, assumptionsWs, 'Assumptions');

      // ─────────────────────────────────────────────────────
      // SHEET 4 — AREA & COST STACK
      // ─────────────────────────────────────────────────────
      const stackRows = [
        ['Area Build-up'],
        [''],
        ['Area', 'Value (sqft)', 'Formula / Note'],
        ['Plot Area', fmtCr(d.plot_area_sqft), 'input'],
        ['Gross FAR Area', fmtCr(d.gross_area_sqft), 'plot × FSI'],
        ['Saleable Super Built-up', fmtCr(d.saleable_area_sqft), 'gross × (1 + loading)'],
        ['Carpet Area', fmtCr(d.carpet_area_sqft), '~65% of gross (industry norm)'],
        [''],
        ['Cost Stack'],
        [''],
        ['Category', 'Value (₹ Cr)', 'Share of Total'],
        ['Land', fmtCr(d.land_cost_cr), null],
        ['Construction', fmtCr(d.construction_cost_cr), null],
        ['Approvals', fmtCr(d.approval_cost_cr), null],
        ['Marketing', fmtCr(d.marketing_cost_cr), null],
        ['Finance', fmtCr(d.finance_cost_cr), null],
        ['GST', fmtCr(d.gst_cost_cr), null],
        ['Stamp Duty', fmtCr(d.stamp_duty_cr), null],
        ['TOTAL', fmtCr(d.total_cost_cr), 1],
      ];
      // Compute shares inline (formula-less) so the spreadsheet is self-contained
      const totalCost = Number(d.total_cost_cr) || 0;
      if (totalCost > 0) {
        const shareRowStart = 11; // Land row
        const shareRowEnd = 18;   // Stamp Duty row
        for (let r = shareRowStart; r <= shareRowEnd; r++) {
          const val = Number(stackRows[r][1]) || 0;
          stackRows[r][2] = Math.round((val / totalCost) * 10000) / 10000;
        }
      }
      const stackWs = buildSheet(stackRows, {
        cols: [{ wch: 28 }, { wch: 18 }, { wch: 28 }],
        freeze: { ySplit: 3 },
        formats: [
          { col: 1, rowStart: 3, rowEnd: 6, z: '#,##0' },       // areas
          { col: 1, rowStart: 11, rowEnd: 19, z: '#,##0.00' },  // cost values
          { col: 2, rowStart: 11, rowEnd: 19, z: '0.00%' },     // shares
        ],
      });
      XLSX.utils.book_append_sheet(wb, stackWs, 'Area & Cost Stack');

      // ─────────────────────────────────────────────────────
      // SHEET 5 — CASH FLOWS (QUARTERLY)
      // ─────────────────────────────────────────────────────
      const cfObj = d.cash_flows || {};
      const quarterly = Array.isArray(cfObj.quarterly) ? cfObj.quarterly : [];
      const yearly = Array.isArray(cfObj.yearly) ? cfObj.yearly : [];

      if (quarterly.length) {
        const qRows = [
          ['Quarterly Cash Flows'],
          [''],
          ['Quarter', 'Label', 'Start', 'End', 'Net (₹ Cr)', 'Cumulative (₹ Cr)'],
        ];
        let cum = 0;
        for (const cf of quarterly) {
          cum += Number(cf.net) || 0;
          qRows.push([
            cf.quarter ?? null,
            cf.label || '',
            ...
            cf.endDate || '',
            Number(cf.net) || 0,
            Math.round(cum * 100) / 100,
          ]);
        }
        qRows.push([]);
        qRows.push(['', '', '', 'Total Inflow', Number(cfObj.summary?.totalInflow) || 0, '']);
        qRows.push(['', '', '', 'Total Outflow', -(Number(cfObj.summary?.totalOutflow) || 0), '']);
        qRows.push(['', '', '', 'Net', (Number(cfObj.summary?.totalInflow) || 0) - (Number(cfObj.summary?.totalOutflow) || 0), '']);

        const qWs = buildSheet(qRows, {
          cols: [{ wch: 10 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 16 }, { wch: 18 }],
          freeze: { ySplit: 3 },
          autofilter: 'A3:F3',
          formats: [
            { col: 4, rowStart: 3, rowEnd: qRows.length - 1, z: '#,##0.00' },
            { col: 5, rowStart: 3, rowEnd: qRows.length - 1, z: '#,##0.00' },
          ],
        });
        XLSX.utils.book_append_sheet(wb, qWs, 'CF Quarterly');
      }

      // ─────────────────────────────────────────────────────
      // SHEET 6 — CASH FLOWS (YEARLY)
      // ─────────────────────────────────────────────────────
      if (yearly.length) {
        const yRows = [
          ['Yearly Cash Flows'],
          [''],
          ['Year', 'Label', 'Start', 'End', 'Net (₹ Cr)'],
        ];
        for (const cf of yearly) {
          yRows.push([
            cf.year ?? null,
            cf.label || '',
            cf.startDate || '',
        ...
        const yWs = buildSheet(yRows, {
          cols: [{ wch: 10 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 16 }],
          freeze: { ySplit: 3 },
          autofilter: 'A3:E3',
          formats: [
            { col: 4, rowStart: 3, rowEnd: yRows.length - 1, z: '#,##0.00' },
          ],
        });
        XLSX.utils.book_append_sheet(wb, yWs, 'CF Yearly');
      }

      // ─────────────────────────────────────────────────────
      // SHEET 7 — SENSITIVITY MATRIX (9x9 IRR grid)
      // ─────────────────────────────────────────────────────
      const sm = d.sensitivity_matrix || {};
      const grid = Array.isArray(sm.irrGrid) ? sm.irrGrid : [];
      if (grid.length) {
        const sellingRates = sm.sellingRates || [];
        const constructionCosts = sm.constructionCosts || [];
        const sRows = [
          ['Sensitivity — IRR (%) by Selling Rate (cols) × Construction Cost (rows)'],
          [''],
          ['Construction \\ Selling', ...sellingRates.map((v) => `₹${Number(v).toLocaleString('en-IN')}/sqft`)],
        ];
        for (let r = 0; r < grid.length; r++) {
          const label = constructionCosts[r] != null ? `₹${Number(constructionCosts[r]).toLocaleString('en-IN')}/sqft` : `Row ${r + 1}`;
          sRows.push([label, ...grid[r].map((v) => (v == null ? null : Number(v)))]);
        }
        const sCols = [{ wch: 22 }, ...Array(sellingRates.length).fill({ wch: 12 })];
        const sWs = buildSheet(sRows, {
          cols: sCols,
          freeze: { ySplit: 3, xSplit: 1 },
          formats: [
            { col: 1, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
            { col: 2, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
            { col: 3, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
            { col: 4, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
            { col: 5, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
            { col: 6, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
            { col: 7, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
            { col: 8, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
            { col: 9, rowStart: 3, rowEnd: 3 + grid.length - 1, z: '0.00"%"' },
          ],
        });
        XLSX.utils.book_append_sheet(wb, sWs, 'Sensitivity');
      }

      // ─────────────────────────────────────────────────────
      // SHEET 8 — NEARBY COMPS
      // ─────────────────────────────────────────────────────
      if (nearbyComps.length) {
        const compRows = [
          ['Nearby Comparable Transactions (5 km radius)'],
          [''],
          ['Project', 'Locality', 'Rate (₹/sqft)', 'Config', 'Launch', 'Possession', 'Source', 'Verified'],
        ];
        for (const c of nearbyComps) {
          compRows.push([
            c.project_name || '—',
            c.locality || '—',
            Number(c.rate_per_sqft) || null,
            c.bhk_config || '—',
            c.launch_year || '—',
            c.possession_year || '—',
            c.source || '—',
        ...
        const compWs = buildSheet(compRows, {
          cols: [{ wch: 26 }, { wch: 22 }, { wch: 14 }, { wch: 10 }, { wch: 10 }, { wch: 12 }, { wch: 18 }, { wch: 10 }],
          freeze: { ySplit: 3 },
          autofilter: 'A3:H3',
          formats: [
            { col: 2, rowStart: 3, rowEnd: compRows.length - 1, z: '#,##0' },
          ],
        });
        XLSX.utils.book_append_sheet(wb, compWs, 'Nearby Comps');
      }

      // ─────────────────────────────────────────────────────
      // SHEET 9 — DD ITEMS
      // ─────────────────────────────────────────────────────
      const ddRows = ddDetailRows || [];
      if (ddRows.length) {
        const ddSheet = [
          ['Due Diligence Items'],
          [''],
          ['Item', 'Category', 'Severity', 'Status', 'Assigned To', 'Due Date', 'Notes'],
        ];
        for (const item of ddRows) {
          ddSheet.push([
            item.item_name || '—',
            item.category || '—',
            item.severity || '—',
        ...
        const ddWs = buildSheet(ddSheet, {
          cols: [{ wch: 32 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 18 }, { wch: 14 }, { wch: 42 }],
          freeze: { ySplit: 3 },
          autofilter: 'A3:G3',
        });
        XLSX.utils.book_append_sheet(wb, ddWs, 'DD Items');
      }

      // ─────────────────────────────────────────────────────
      // SHEET 10 — RISK FLAGS
      // ─────────────────────────────────────────────────────
      const riskRows = riskDetailRows || [];
      if (riskRows.length) {
        const riskSheet = [
          ['Risk Flags'],
          [''],
        ...
        const rWs = buildSheet(riskSheet, {
          cols: [{ wch: 32 }, { wch: 14 }, { wch: 16 }, { wch: 14 }, { wch: 60 }],
          freeze: { ySplit: 3 },
          autofilter: 'A3:E3',
        });
        XLSX.utils.book_append_sheet(wb, rWs, 'Risks');
      }
      ...
      const metaRows = [
        ...
        [''],
        ['Field', 'Value'],
        ['Report Type', 'Deal Investor Package'],
        ['Platform', 'REDIP — Real Estate Development Intelligence'],
        ['Deal ID', d.id],
        ['Generated By', req.user.name],
        ['Generated At', new Date().toISOString()],
        ['AI Insights', insights.available ? `Available (confidence: ${insights.confidence || 'medium'})` : `Unavailable — ${insights.reason || 'n/a'}`],
        [''],
        ['Disclaimer', 'This export is generated from current deal assumptions. All figures are in INR Crore unless stated otherwise. AI-generated content is informational only. Verify all inputs and risks before any investment decision.'],
      ];
      const metaWs = buildSheet(metaRows, {
        cols: [{ wch: 20 }, { wch: 80 }],
      });
