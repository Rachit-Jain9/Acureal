...
//
// Pure deterministic JS — no AI involvement. Mirrors frontend/src/utils/
// buildEnvelope.js verbatim (only the export syntax differs); change one,
// change both, and run the parity test in backend/tests/buildEnvelope.parity.test.js.
//
// All figures come from the OPERATIVE Revised Master Plan 2015, Volume III
// (Zoning of Land Use and Regulations; Bangalore Development Authority,
// approved vide G.O. UDD 540 BEM AA SE 2004 dated 22-06-2007):
//   - FAR / ground coverage: per-zone fsi_road_width_rules JSONB on master_plan_zones.
//   - Setbacks, height <= 11.5 m: Table 8 (Chapter 3.1) — percentage-based on
//     the site's width (left/right) and depth (front/rear); plot > 4000 sqm = 5 m all sides.
...
const HIGH_RISE_SETBACK_BY_HEIGHT = [
...
function lowRiseSetback({ siteWidthM, siteDepthM, plotAreaSqm } = {}) {
...
function highRiseSetback(buildingHeightM) {
...
function setbacksForBuilding({ buildingHeightM, siteWidthM, siteDepthM, plotAreaSqm } = {}) {
  const h = toNumber(buildingHeightM);
...
function deriveSiteDimensions({ plotAreaSqm, siteWidthM, siteDepthM } = {}) {
  const area = toNumber(plotAreaSqm);
  const w = toNumber(siteWidthM);
  const d = toNumber(siteDepthM);
...
module.exports = {
  HIGH_RISE_SETBACK_BY_HEIGHT,
  lowRiseSetback,
  highRiseSetback,
  setbacksForBuilding,
  deriveSiteDimensions,
