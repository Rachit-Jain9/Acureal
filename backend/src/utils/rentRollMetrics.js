...
const PHYSICAL_STATUSES = new Set(['occupied', 'notice_served']);
const CONTRACTED_STATUSES = new Set(['occupied', 'notice_served', 'committed']);
...
const monthlyMarketRentPaise = (r) => {
 ...
 */
const computeLeaseMetrics = (records, parent = {}, opts = {}) => {
...
const validateLeaseRoll = (records, parent = {}, financialInputs = null) => {
  const warnings = [];
  const rows = (records || []).filter((r) => r && !r.deleted_at);
...
const toLeaseExtract = (records, opts = {}) => {
...
const SALE_SOLD_STATUSES = new Set(['booked', 'sold', 'registered']);
 ...
 */
const computeSaleMetrics = (records, parent = {}, opts = {}) => {
  const asOf = parseDate(opts.asOf) || parseDate(parent.as_of_date) || new Date();
  const rows = (records || []).filter((r) => r && !r.deleted_at);
...
const validateSaleRoll = (records, parent = {}) => {
  const warnings = [];
  const rows = (records || []).filter((r) => r && !r.deleted_at);
  const add = (code, message, recordIds = []) => warnings.push({ code, message, recordIds });
 ...
 * @param {object} [opts]         { asOf?: Date|string }
 */
const computeHotelMetrics = (recordsByKind = {}, parent = {}, opts = {}) => {
  const asOf = parseDate(opts.asOf) || parseDate(parent.as_of_date) || new Date();
...
const validateHotelRoll = (recordsByKind = {}) => {
  const warnings = [];
  const live = (r) => r && !r.deleted_at;
  const add = (code, message, recordIds = []) => warnings.push({ code, message, recordIds });
