...
// `redip_meta` sheet that stamps the template version, family, and the exact
// column-key ↔ header ↔ sheet map. The (forthcoming) deterministic importer
// reads redip_meta to map each sheet back to its kind and each column back to
// its DB field with zero guessing — so a filled template round-trips exactly.
//
...
const buildKindSheet = (workbook, kind) => {
  ...
  styleTitle(sheet.getCell(1, 1), `REDIP | ${sheetName} — import template`);
  ...
  styleInstruction(sheet.getCell(2, 1),
    ...
    + 'Do not rename or reorder the header row, and do not edit the hidden redip_meta sheet.');
...
const buildMetaSheet = (workbook, family, sheetSpecs) => {
  ...
  meta.getCell('A1').value = 'REDIP DEAL-REGISTER IMPORT TEMPLATE — do not edit or delete this sheet.';
...
const templateFileName = (family, dealName) => {
  ...
  return `REDIP-${deal}-${fam}-template.xlsx`;
};
