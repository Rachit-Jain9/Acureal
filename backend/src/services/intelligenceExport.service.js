...
// Draw the navy top bar with REDIP branding and a city/date subtitle. Mirrors
...
const drawFooter = (page, fonts, { generatedBy, pageNum, pageTotal }) => {
  ...
  page.drawText(`REDIP Market Intelligence Tear-Sheet  -  generated by ${safeText(generatedBy)} on ${new Date().toISOString().slice(0, 10)}`, {
...
const buildIntelligenceTearSheet = async ({ city = 'Bengaluru', generatedBy = 'Unknown user' } = {}) => {
  ...
  pdfDoc.setTitle(`REDIP Market Intelligence Tear-Sheet - ${city}`);
  pdfDoc.setAuthor('REDIP');
  pdfDoc.setSubject(`${city} verified market benchmarks - generated ${new Date().toISOString()}`);
  pdfDoc.setProducer('REDIP intelligenceExport.service');
