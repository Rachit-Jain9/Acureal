 ...
 *     • Navy header bar (Acureal / Investor Tear Sheet / deal name)
...
const drawFooter = (page, fonts, { generatedBy, pageNum, pageTotal }) => {
  ...
  page.drawText(
    `Acureal Investor Tear-Sheet  -  generated by ${safeText(generatedBy)} on ${new Date().toISOString().slice(0, 10)}  -  AI-assisted, requires human review`,
 ...
 */
async function buildDealTearSheet({ dealId, generatedBy = 'Acureal user' } = {}) {
  ...
  y2 = drawSectionHeader(page2, fonts, {
    ...
    title: 'AI Synthesis',
    sub: synthesis.body
      ? 'Excerpted from the latest persisted draft. Open the deal in Acureal for the full memo.'
  ...
  return {
    ...
    fileName: `acureal-${(deal.name || 'deal').replace(/[^a-z0-9_-]/gi, '_').toLowerCase().slice(0, 60)}-tear-sheet-${today}.pdf`,
