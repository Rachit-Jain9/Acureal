...
const buildInputsSheet = (workbook, ctx) => {
  ...
  // Indian residential developers price on milestone-anchored escalation:
  // ~25-35% cumulative lift from launch to handover, broken by milestones
  // (plinth = +5%, each slab = +1-2%, pre-OC = +5%, handover = market).
  // The current continuous compound model `(1+EscalationPct)^q/4`
  // understates early-quarter pricing.
  //
  // PR-I11 adds disclosure-only inputs documenting the escalation curve.
  // The Phasing P&L's Quarter sales formula continues to use EscalationPct.
  // The derived row computes the equivalent continuous-compound rate the
  // operator can paste into EscalationPct to match the milestone total.
  //
  // Visible only for residential / villas / mixed_use (where milestone
  // pricing is conventional).
  ...
  // Plotted developments sell plot-by-plot. Big plots clear slower (higher
  // ticket, smaller buyer pool); small plots clear fast. The uniform
  // SalesVelocityPct misrepresents premium plot mixes. PR-I14 adds
  // disclosure inputs documenting the plot-size distribution + absorption
  // period. Informational — Phasing continues to use SalesVelocityPct;
  // operator references PlotAbsorptionMonths when setting velocity.
  //
  // Visible only for plotted_development.
  ...
  // on a single parcel. A single SellRatePerSqft can't capture this. PR-I15
  // adds disclosure inputs for each component's share + revenue rate.
  // Derived "Blended Sale Rate" gives the operator a number to paste into
  // SellRatePerSqft. Informational.
  //
  // Visible only for mixed_use / redevelopment.
  ...
  // Raw-land deals are pre-construction: operator buys at one price, holds
  // through 1-3 entitlement stages (title diligence → conversion →
  // layout approval → sale-ready), resells at approval-uplift price. The
  // construction-phasing model is wrong here. PR-I16 adds disclosure
  // inputs documenting the entitlement pipeline. Informational only.
  //
  // Visible only for raw_land.
