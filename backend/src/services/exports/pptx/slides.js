  ...
  addBulletList(slide, [
    ...
    context.readiness?.readiness_pct != null
      ? context.approvalSummary?.required
        ...
        : `Readiness stands at ${context.readiness.readiness_pct}%, and required approvals have not yet been fully tagged in Acureal.`
...
const renderInvestmentHighlights = (pptx, slide, context, pageNumber, totalSlides) => {
  ...
  const cards = context.highlightCards.length
    ...
    : [{ title: 'Deal context', detail: 'Stored Acureal data is currently limited, so the highlight set will expand as approvals, comps, and underwriting inputs are added.' }];
...
const renderReadiness = (pptx, slide, context, pageNumber, totalSlides) => {
  ...
  if (diligenceTableRows) {
    ...
    slide.addText('No open diligence items are currently linked in Acureal.', {
...
const renderCashFlowSensitivity = (pptx, slide, context, pageNumber, totalSlides) => {
  ...
  if (scenarioRows.length > 0) {
    ...
    slide.addText('Run the financial kernel with Base / Bull / Bear sensitivity overrides on the Financial Engine tab in Acureal to populate this panel.', {
...
const renderDisclaimer = (pptx, slide, context, pageNumber, totalSlides) => {
  ...
  // Heading
  slide.addText('Acureal — INTERNAL INVESTMENT MATERIAL', {
  ...
  // Lead paragraph
  slide.addText(
    'This deck is generated by Acureal from stored deal data and verified market sources. ' +
  ...
  addBulletList(slide, [
    'Acureal does not warrant zoning, legal title, RERA registration, encumbrance status, or approval status. Independent verification through Karnataka land-records (Bhoomi / Kaveri portal) and Karnataka RERA is required before any investment decision.',
    'Comparables carry per-row source and verification status; only rows marked Verified have been confirmed in Acureal — treat all others as context. Confirm freshness and applicability against external sources before relying on the comp set.',
  ...
  // Footer. Carries the kernel computation reference so a deck circulated
  // ahead of an IC can be tied back to the exact signed computation that
  // produced its figures. No AI mention anywhere in PPTX — product policy.
  slide.addText(
    `Generated by ${context.brandName} on ${formatDate(context.generatedAt)} for ${context.generatedFor}.`
    + (context.computationRef?.ref ? `  ·  Kernel computation ${context.computationRef.ref}` : ''),
    {
 ...
 *   - Deal record           — saved on the deal row in Acureal
