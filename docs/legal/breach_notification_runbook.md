...
2. Estimate blast radius: which tables, which orgs, which users are potentially affected.
3. Preserve logs (do not rotate; freeze backup retention).
4. Pause affected credentials and rotate:
   - `JWT_SECRET`
...
- Patch root cause.
- Restore from clean backups if needed; verify hashes / checksums match pre-incident state.
...
| Backup restore drill | founder | — |
| Vercel admin access list | founder | — |
| Supabase admin access list | founder | — |
| Anthropic / Google billing alerts configured | founder | — |
