// Mirror of backend/src/utils/siteYield.js. Keep in lockstep — change one,
// change both, and run the parity test in backend/tests/siteYield.parity.test.js.
//
...
//
// Takes a regulatory ENVELOPE (FAR / coverage / height already computed by
// buildEnvelope.js or parcelBuildability.js) plus EDITABLE programme
// ASSUMPTIONS and produces a screening development programme for the deal's
// asset class — realized GFA, the binding constraint, an area schedule, a
// unit / key / plot count and a parking demand — then emits `programme` +
// `buildability` objects in the exact shapes programmeToInputs.js consumes so
// a chosen scheme flows one click into the Financial Engine.
//
// Provenance: every ratio (loading, efficiency, parking norm, unit mix, plot
// size, saleable-land fraction) is a SCREENING DEFAULT, overridable via
// `assumptions`, never asserted as a statutory fact.
...
export const FAMILY_BY_ASSET_CLASS = {
...
export const resolveFamily = (assetClass) => FAMILY_BY_ASSET_CLASS[assetClass] || 'residential';
...
export const DEFAULT_RESIDENTIAL_MIX = [
...
export const DEFAULT_BUILD_COST_PER_SQFT = {
  residential: 2600,
  commercial: 3000,
  industrial: 1800,
  hospitality: 4200,
...
export function largestRemainderCounts(targets) {
...
export function computeRealizedGfa(env, { floorToFloorM } = {}) {
...
export function computeSiteYield(input = {}) {
