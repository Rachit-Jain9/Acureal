...
import {
  ...
  BookOpen, Ruler, AlertTriangle, Shield, ExternalLink, FileText,
} from 'lucide-react';
...
/**
 * The gazette evidence explorer — extracted from ParcelEvidenceRoomPage so the
 * SAME cited chain (street → ward → register → UAV zone → guidance band →
 * exact gazette page) serves two surfaces:
 *
 *   • /dashboard/parcel-evidence (authenticated) — `useLookup` is the app's
 *     useStreetLookup, and the terminal CTA starts a deal;
 *   • /parcel (public) — `useLookup` is usePublicStreetLookup on the
 *     unauthenticated client, and the terminal CTA converts to signup.
 *
 * `useLookup` is a HOOK passed as a prop; it is called unconditionally at the
 * top level with a stable identity (both callers pass a module-level hook), so
 * the rules of hooks hold. `terminalCta` renders inside the deal-gated FAR
 * node — the honest boundary where the chain stops — so each surface owns its
 * own conversion without forking the evidence UI.
 */
...
function EvidenceChain({ row, sourceDocument, terminalCta }) {
    ...
    <div className="relative">
      ...
      >
        ...
        <div className="rounded-editorial border border-dashed border-hairline-strong bg-bg-secondary/50 p-3.5">
          <div className="text-[10px] uppercase tracking-[0.13em] text-content-muted font-semibold">
            Statutory zoning, FAR &amp; indicative buildable
          </div>
          {terminalCta}
        </div>
      </div>
...
export default function ParcelEvidenceExplorer({ useLookup, limit = 40, terminalCta = null }) {
  ...
  const { data, isLoading, isError, isFetching } = useLookup({ search: debounced, limit });
          ...
          <div className="min-w-0">
            {focused ? (
              <Card elevated className="p-6">
                ...
                <EvidenceChain row={focused} sourceDocument={sourceDocument} terminalCta={terminalCta} />
              </Card>
            ) : (
