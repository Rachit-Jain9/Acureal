...
CREATE TABLE IF NOT EXISTS financial_scenarios (
  ...
  scenario         VARCHAR(32) NOT NULL,        -- 'base' | 'bull' | 'bear' | custom
  label            VARCHAR(100),
...
-- Backfill organization_id from parent deal for any pre-existing rows
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'financial_scenarios'
  ) THEN
...
CREATE TABLE IF NOT EXISTS waterfall_distributions (
  ...
  deal_id          UUID NOT NULL REFERENCES deals(id) ON DELETE CASCADE,
  organization_id  UUID,
  kind             VARCHAR(16) NOT NULL,        -- 'jda' | 'jv'
  inputs           JSONB NOT NULL DEFAULT '{}'::jsonb,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'waterfall_distributions'
  ) THEN
...
CREATE TRIGGER trg_financial_scenarios_set_org
  BEFORE INSERT OR UPDATE OF deal_id ON financial_scenarios
  FOR EACH ROW EXECUTE FUNCTION set_organization_id_from_deal();

DROP TRIGGER IF EXISTS trg_waterfall_distributions_set_org ON waterfall_distributions;
CREATE TRIGGER trg_waterfall_distributions_set_org
  BEFORE INSERT OR UPDATE OF deal_id ON waterfall_distributions
  FOR EACH ROW EXECUTE FUNCTION set_organization_id_from_deal();
